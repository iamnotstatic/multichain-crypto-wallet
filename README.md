...
- The **params** parameter takes in the parameters of the smart contract function if it requires any. It must be in the same order as of the contract address. If the smart contract function does not require any parameters, leave it as an empty array.

The optional parameters that the object takes in are: value, contractAbi, gas price, gas limit, nonce, and private key.

- The value is the amount of ETH. Some smart contracts require you to specify.
...
- The nonce is the number of transactions that have been sent from the source address and is used to ensure that the transaction is unique. The transaction is unique because the nonce is incremented each time a transaction is sent.
- The private key is a string parameter that can be passed in for a wallet. It is used to sign the transaction. This parameter is not needed when calling a smart contract with the read function.
#### Response
